...
With `listen: true`, the store reacts to changes from ANY adapter in the chain:
...
// Changes in localStorage trigger store update
// Changes in sessionStorage also trigger store update
// Store always re-reads from the chain (respecting priority order)
```
