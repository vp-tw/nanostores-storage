...
<Aside type="tip" title="Why null instead of undefined?">
  When a key doesn't exist, `get(key)` returns `null` (not `undefined`). This aligns with the Web
  Storage API (`localStorage.getItem()`) and `createStorageStore`, allowing for consistent
